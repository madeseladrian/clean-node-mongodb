...
describe('DbAddAccount Usecase', () => {
  test('1 - Should call Hasher with correct plaintext', async () => {
  ...
  test('5 - Should return true on success', async () => {
    const { sut } = makeSut()
    const isValid = await sut.add(mockAddAccountParams())
    expect(isValid).toBe(true)
  })

  test('6 - Should return false if AddAccountRepository returns false', async () => {
    const { sut, addAccountRepositorySpy } = makeSut()
  ...
  test('7 - Should return false if CheckAccountByEmailRepository returns true', async () => {
  ...
  test('8 - Should call LoadAccountByEmailRepository with correct email', async () => {
    const { sut, checkAccountByEmailRepositorySpy } = makeSut()
    const addAccountParams = mockAddAccountParams()
    await sut.add(addAccountParams)
